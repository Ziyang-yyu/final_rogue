...
def place_entities(i: int, room: RectangularRoom, dungeon: GameMap, floor_number: int) -> None:
    ...
    agent_id, agent_num = setup_game.MainMenu().get_agent()
    if agent_num >= 4:
        monsters.append(entity_factories.miner3)
        for i in range(agent_num-1):
            agent_id = random.randint(0,3)
            if agent_id == 0:
                monsters.append(entity_factories.miner0)
            elif agent_id == 1:
                monsters.append(entity_factories.miner1)
            elif agent_id == 2:
                monsters.append(entity_factories.miner2)
            elif agent_id == 3:
                monsters.append(entity_factories.miner3)
            elif agent_id == 4:
                monsters.append(entity_factories.miner4)
    else:
    ...
    # for training 3 items
    for i in range(3):
          items.append(entity_factories.health_potion)
...
def generate_dungeon(
    ...
    # restrict number of bfs chasers
    for i in range(max_rooms):
        ...
        place_entities(i,new_room, dungeon, engine.game_world.current_floor)
